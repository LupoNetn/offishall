...
const initialState = {
  name: "",
  price: "", // Changed to string for consistency
  category: "",
  description: "",
  image: null,
}
...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let image_url = null;
      if (state.image) {
        const fileExt = state.image.name.split(".").pop();
        const fileName = `${Math.random()}.${fileExt}`;
        const { error: uploadError, data } = await supabase.storage
          .from("product-images")
          .upload(fileName, state.image);
      ...
      state.name = "";
      state.price = ""; // Reset price to empty string
      state.category = "";
      state.description = "";
      state.image = null; // Reset image to null
      setShowForm(false);
      fetchProducts();
  ...
  const handleChange = (e) => {
   ...
   dispatch({
    ...
    payload: name === "image" ? files[0] : value,
   })
  };
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-950/50">
                  <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider py-4 px-4 sm:px-6">
                    Product
                  </th>
                  <th className="hidden sm:table-cell text-left text-xs font-medium text-gray-400 uppercase tracking-wider py-4 px-4 sm:px-6">
                    Category
                  </th>
                  <th className="text-left text-xs font-medium text-gray-400 uppercase tracking-wider py-4 px-4 sm:px-6">
                    Price
                  </th>
                  <th className="hidden lg:table-cell text-left text-xs font-medium text-gray-400 uppercase tracking-wider py-4 px-4 sm:px-6">
                    Description
                  </th>
                  <th className="text-right text-xs font-medium text-gray-400 uppercase tracking-wider py-4 px-4 sm:px-6">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="py-4 px-4 sm:px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-lg bg-gray-800 flex items-center justify-center overflow-hidden border-2 border-gray-700">
                          {product.image_url ? (
                           <img
                           src={`https://your-project-id.supabase.co/storage/v1/object/public/product-images/${product.image_url}`}
                           alt={product.name}
                ...
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-700 border-dashed rounded-lg">
                  <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-400">
                      <label
                        htmlFor="file-upload"
                        className="relative cursor-pointer rounded-md font-medium text-orange-500 hover:text-orange-400"
                      >
